...
__version__ = '0.0.1'
__all__ = [
    'SetDebug', 'ResetDebug', 'PrintDebug', 'PrintError','PrintLog',
    'PrintWarning','SetLogFile',"AdsVar",
    'iso8601_2_datetime','rec_2_json','json_2_rec'
]
...
"""ISO 8601 2 date time string parsing

Basic usage:
>>> import iso8601
>>> iso8601_2_datetime("2007-01-25T12:00:00Z")
...
def iso8601_2_datetime(datestring):
    ...
    def parse_timezone(matches, default_timezone=UTC):
        ...
        if matches["timezone"] in ("Z","+00:00"):
...
"""
rec_2_json  : converteer de timestamps in dictionarie naar het isoformaat string zodat we
              dictorie kunnen opslaan met json
...
def rec_2_json(rec):
    ...
    d={}
    for field in rec:
        if isinstance(rec[field],datetime):
            n['__json__'+field]=rec[field].isoformat()
            d[field]=rec[field]
    for field in n:
        rec[field]=n[field]
    for field in d:
...
"""
json_2_rec  : converteer de isoformaat timsestanmp string uit json record ,
                naar datetimeformaat nodig om verder verwerking

"""
def json_2_rec(rec):
    from datetime import datetime
    from adson import iso8601_2_datetime
    n={}
    d={}
    for field in rec:
        if field[0:8]=="__json__":
            n[field[8:]]=iso8601_2_datetime(rec[field])
            d[field]=rec[field]
    for field in n:
        rec[field]=n[field]
    for field in d:
